...
~~~text
sRGB 图像纹理在 CUDA texture read 中解码后过滤
  → 在线性 RGB 中计算路径贡献
...
alpha 通道不代表亮度，不做 sRGB 解码。标记为 `linear` 的图像与 Python API 传入的 `base_color`、`emission` 常量也直接作为线性数值使用。设备端 `sample_texture` 不再手工解码，也不预先截断 UV；clamp、repeat 或 mirrored-repeat 由同一 texture object 的 address mode 处理。
...
这个顺序避免了“先在 sRGB 码值中插值、再解码”导致的中间色偏暗。主机侧同时绑定 address mode、linear filter、normalized-float read 和 sRGB 开关：
...
`td.sRGB` 只在 8 bit image texture 被声明为 sRGB 时开启；constant texture 和 `linear` image 保持关闭。因此 color space 是资源创建时的类型化属性，而不是着色器在采样后猜测的分支。
...
- **normal**：首命中 opaque/粗糙表面的 $\mathbf n_s^{\mathrm{eff}}$，或光滑 dielectric/water 的 $\mathbf n_g$，在相机基 $(\mathbf u,\mathbf v,\mathbf w)$ 上的三个投影。
...
`guide_written` 把写入限定在首个有效交点。`resolved_material` 先将 PBR packed MR 纹理的 G/B 通道乘入 roughness/metallic factor，`material_color` 再得到 base-color 纹理调制后的线性 `base_color`。代码检查材质是否为 `roughness <= 0` 的 dielectric/water：是则选 $\mathbf n_g$，否则选 $\mathbf n_s^{\mathrm{eff}}$，再对相机基 $\mathbf u$、$\mathbf v$、$\mathbf w$ 投影。PBR normal map 已在 closest-hit 阶段进入 `hit.normal`，所以 normal guide 与实际连续 BSDF 波瓣使用同一根有效着色法线。外层样本循环结束后，这两个和与 beauty 一样除以 spp。
