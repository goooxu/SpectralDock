...
降噪是 `render_optix` 中的可选分支。默认先让 `final_beauty` 指向路径追踪产生的原始 `beauty`；只有场景或 CLI 启用 `denoise` 时才调用 [`run_denoiser`](../../src/optix_renderer.cpp)，并把 `final_beauty` 改为降噪输出。因此两条路径的分歧只在是否经过 OptiX Denoiser，后面共享同一套曝光和显示变换。
