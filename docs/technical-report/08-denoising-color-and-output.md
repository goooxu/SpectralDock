...
`guide_written` 把写入限定在首个有效交点；albedo 直接累加纹理调制后的 `base_color`。代码先检查材质是否为 `roughness <= 0` 的 dielectric/water：是则选 $\mathbf n_g$，否则选 $\mathbf n_s^{\mathrm{eff}}$，再对相机基 $\mathbf u$、$\mathbf v$、$\mathbf w$ 投影。这让连续表面跟随视觉波瓣，而光滑介质跟随真正用于 Fresnel/Snell 的 delta 法线。外层样本循环结束后，这两个和与 beauty 一样除以 spp。
