...
Rocket Test Stand 的正式配置使用 2048 spp 且关闭 Denoiser。原因不是 Denoiser 不能处理亮色，而是当前 albedo/normal guide 只描述首次表面，没有体积特征或透射 guide；让正式图保留原始 Monte Carlo 结果，能更直接暴露硬支撑边界、分层和 firefly。
