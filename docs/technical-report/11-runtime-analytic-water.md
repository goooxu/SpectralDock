...
完整水面仍进入 OptiX GAS，但不是三角网格。最短波长的一半决定 tile 尺寸；每个 tile 只提供覆盖局部最大振幅的保守 AABB，全部 tile 共享同一组波浪参数和 SBT 数据。每个 tile box 完成既有 overlap 后，六个边界还在上传前各向外舍入一个 float ULP，避免大坐标下固定 overlap 小于一个 ULP 而被量化掉。BVH 因而负责快速剔除，真正曲面仍由同一个解析函数定义。
...
`water_solver_epsilon = 1e-4` 是这套解析求根器自己的世界空间容差，用于端点交叉探测和无法分辨的近切 enter/exit 对；它不再传给表面 ray spawn 或 OptiX 的 `tmin/tmax`。因此后续射线按各自的 `position_error` 外推，而水面求根的尺度边界仍由本章的独立安全门约束。
...
这里刻意保留两个距离：`shadow_distance` 是数值可见性段；对有限表面灯，它从外推 origin 到向连接内部外推后的灯面端点重算，用于 OptiX 遮挡查询。`medium_distance` 是未偏移表面点到原始灯点的物理传播距离，用于 Beer 衰减。体积跟踪也使用后一类未偏移段，ray-spawn 的两端偏移不会凭空缩短吸收路径。
...
shadow origin 与后续 radiance origin 都走同一 helper：它把 `SurfaceHit.position_error` 投影到 $\mathbf n_g$，正负侧由真实新方向决定，再按定向法线的各坐标符号用 `nextafterf` 向外舍入。这既避免透射连接从错误介质侧立即自交，也防止普通平滑 mesh 被倾斜着色法线推过真实表面；偏移量来自交点误差界，不再是固定世界空间长度：
...
摘录展示 X 分量；Y/Z 分量执行完全相同的正负无穷方向外推，随后返回新 origin。这里的 `direction` 只决定 $\mathbf n_g$ 的哪一侧，偏移幅度由 `position_error` 决定。
