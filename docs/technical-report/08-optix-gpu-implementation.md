...
前七章已经给出渲染器的数学、采样与几何。本章把一个典型 OptiX 应用从**构建期**到**运行期**、再到资源销毁的完整流程映射到 SpectralDock。几何上传和加速结构的细节在[第 7 章](07-geometry-visibility-and-bvh.md)，降噪与显示输出在[第 9 章](09-denoising-color-and-output.md)；本章是贯穿三章的流程总线。
...
Python binding 只把用户选择的非负 device ordinal 写入 `settings.device`，不接受或创建 CUDA stream、`CUcontext` 或 OptiX context。`render_optix` 在 native 侧调用 `cudaSetDevice(settings.device)`，以 `cudaFree(nullptr)` 触发所选 device 的 CUDA primary context 初始化，再由 RAII `Stream` 调用 `cudaStreamCreate`。随后它通过 `cuCtxGetCurrent` 借用当前 `CUcontext`，初始化 OptiX，并按 `validation` 设置日志与验证级别后创建自己的 OptiX device context。函数不会销毁借来的 CUDA context，但会销毁自己创建的 stream 和 OptiX context。
...
八个 dispatch slot 是 sphere、triangle、disk、cylinder、parabola、mesh、water surface 和 solid sphere。rectangle（包括带 alpha 裁剪的版本）在主机端变为 triangle；mesh 也使用内建三角形求交，但从 SBT 读取独立的顶点属性缓冲区。disk、cylinder、parabola、解析水面和实心 sphere 使用项目自定义 intersection；普通 sphere 与 triangle/mesh 使用 OptiX 内建求交。实心 sphere 只服务于含水场景中的 dielectric 闭合边界，原因见[第 11 章第 4 节](11-runtime-analytic-water.md#水中-dielectric-sphere-为什么使用自定义实心边界)。
...
CPU Scene / MeshResource
  → CUDA DeviceBuffer（position、normal、UV、Mikk face-corner tangent、index、可选逐三角形材质 ID）
  → OptixBuildInput
...
[第 7 章第 6 节](07-geometry-visibility-and-bvh.md#6-spectraldock-的-gas-与-ias)给出同步源码片段：`build_mesh` 只把 position/index 的 CUDA 设备指针写入三角形 `OptixBuildInput`，`compact_gas` 按收益决定是否压缩；`build_ias` 再上传 `OptixInstance` 数组，并经同一 `compact_gas` 路径构建和按收益压缩 IAS。normal、UV、逐 face-corner 的 MikkTSpace `corner_tangents`（`float4` 的第四分量保存 handedness）和逐三角形材质 ID 都不参与 BVH 构建，而是 closest-hit 从独立 CUDA buffer 读取的着色属性。

同一 mesh resource 的多个对象共享一份 GAS，避免重复上传和构建；严格 `usemtl` resource 还共享固定的逐 primitive 材质表。每个实例仍有自己的变换和 SBT 偏移。另一套槽映射意味着另一份 mesh resource 和 GAS。GAS/IAS 回答“射线可能命中什么”，下一节的 SBT 回答“命中后执行哪个程序并读取哪份对象数据”。
...
IAS instance 的 `sbtOffset = object index * 2`；`optixTrace` 再用 ray type 选择 radiance 或 shadow record。每条 record 包含与 primitive 类别匹配的程序 header，以及该对象的 `GeometryData`、legacy 正反面材质索引、alpha 和灯索引；mesh record 还保存顶点、法线、UV、Mikk face-corner tangent、索引及可选逐三角形材质 ID 的设备指针。完整材质数组由 `LaunchParams.materials` 提供。

材质槽数量不会改变 SBT record 数。mapped mesh 与 legacy mesh 一样仍是每个 object 两条 records，GAS 的三角形构建输入也保持 `numSbtRecords = 1`；closest-hit 用 OptiX primitive index 查询材质 ID buffer，再索引全局 `MaterialData`。因此一个十槽 Sparky 仍是一份 GAS、一个 triangle build-input SBT record 和两条 object hitgroup records，而不是十份 GAS 或每槽一条 record。mapped triangle 的正反面使用同一个槽材质；legacy mesh 才从 object record 选择 `front_material`/`back_material`。alpha texture 与 cutoff 在两种模式中都保持 object-wide。

PBR 也沿用这条分发链。每个 `MaterialData` 以 `texture_index` 保存独立的 base-color 槽，以 `metallic_roughness_texture_index` 和 `normal_texture_index` 保存 packed MR 与 normal 槽，并携带标量 `normal_scale`；纹理对象仍来自全局 `LaunchParams.textures`。PBR 是 raygen/closest-hit 内的材质分支，不是新的 primitive 类别，所以它不增加 program group、GAS build-input SBT record 或 object hitgroup record；normal map 所需的 `corner_tangents` 只是既有 mesh record 中多出的属性指针。
...
外层循环对应对象，内层依次追加 radiance 和 shadow record，正好实现上式。共享同一 mesh GAS 的实例仍由各自的 `OptixInstance.transform` 保留变换，并由各自的 SBT record 保留 object-wide 数据；严格映射材质表则随 resource 共享。`make_sbt` 还各自创建一条 raygen record 和两条 miss records，并把这些设备地址写入 `OptixShaderBindingTable`。

## 7. LaunchParams：一次 launch 的全局上下文

SBT 完成后，主机分配并清零输出与统计缓冲，再填写 `LaunchParams`。为避免把它误解成只有相机和图像尺寸的参数包，可以按用途分成四组：
...
`traversable` 是遍历入口，三个图像指针是 raygen 的输出，尺寸与 spp 决定工作量，`max_depth` 控制路径循环上限，`seed` 和相机决定初始样本。有限面光/体积光通过同一浮点 CDF 及全局索引表随机选择，point/directional delta 灯则通过独立索引表逐盏确定性求值；环境 row/conditional CDF 又是第三套分布。结构体整体上传到设备后，设备程序通过常量 `params` 访问。
...
含水场景也使用同一个二值查询。粗糙介电 NEE 若连接当前界面的透射侧，raygen 在发射 shadow 前把 origin 移到正确一侧，并在 `MediumState` 副本中切换这**一次**边界；shadow 段可见后再按副本顶层介质计算 Beer。连接线上若遇到下一层水或玻璃，any-hit 会像处理不透明表面一样终止查询，因为一条直线不能代表经过第二个折射事件的光路。数学、介质栈与 MNEE 边界见[第 11 章第 6、7 节](11-runtime-analytic-water.md#6-粗糙水面的-nee只连接当前散射事件)。
