...
Radiance Pavilion 把场景放在一张 2048×1024 的程序化 HDR 日落海岸环境中，JSON 的 `lights` 为空。低角度金色夕阳、暖色分层云、冷色天顶、暗青海面、太阳反光带与远岛剪影既出现在相机 miss 和镜面反射中，也从无限远方向照亮中央吉祥物及四件采用漫反射、粗糙金属、光滑金属和介电材质的户外观测装置。夕阳热点集中、天空补光宽广，使它既是场景的唯一光源，也是观察重要性采样收益的教学输入。要让这种照明既正确又低方差，工程上必须同时解决四件事：解码线性辐亮度、在球面与二维图像间一致映射、按亮度和立体角构造 PDF，以及让 NEE 与 BSDF miss 共享同一个 PDF 做 MIS。
...
若有 $N$ 个有限灯，先在主机构造一个全局基础分布 $q_i^G$。uniform 模式令 $q_i^G=1/N$。importance 模式先为每个灯计算非负功率代理 $w_i$：
...
全零代理退化为均匀选择。场景最多有 4096 个显式灯，最终 $q_i^G$ 取自 float CDF 的实际区间。
...
单一全局功率分布会让强月光几乎占满所有水面样本，使粗糙透射侧的弱水下灯仍难以被选到。设全局分布为 $q_G(i)$，均匀索引分布为 $q_U(i)=1/N$。设备端按当前顶点采用以下策略：

- 粗糙 water 顶点确定性地取两份有限灯样本：$I_G\sim q_G$ 和 $I_U\sim q_U$；
- 已在任意介质内的其他顶点只取一份 $q_U$ 样本；
- 其余空气中顶点只取一份 $q_G$ 样本。

令 $c_i(\xi)=p(\xi\mid i)$ 是选中第 $i$ 盏灯后的条件采样 PDF，$C(i,\xi)$ 是不含采样分母的路径贡献，并定义

$$
p_G=q_G(i)c_i(\xi),\qquad
p_U=q_U(i)c_i(\xi),\qquad
p_L=p_G+p_U.
$$

若灯没有可命中的绑定表面，或当前深度没有下一条 BSDF 射线，两份灯样本分别除以 $p_L$；它们的期望在每个灯上相加成 $(p_G+p_U)/p_L=1$。若绑定表面还可由 BSDF 以方向密度 $p_B$ 命中，两份 direct 样本分别除以 $p_L+p_B$，BSDF continuation 命中时也除以同一总密度。三份期望相加成

$$
\frac{p_G+p_U+p_B}{p_G+p_U+p_B}=1.
$$

因此两份灯贡献直接相加，**不乘 0.5**；可命中绑定灯再由 BSDF 端补齐 balance 权重。这是每个提议固定一份样本的 deterministic multi-sample MIS，不是先以 50/50 随机选一个提议。它不修改灯辐亮度，只用固定的额外样本换取更低方差。
...
`finite_light_selection_pdf` 对两份 water 样本都返回 balance 分母 $q_G+q_U$，而样本索引本身分别由 power CDF 和 uniform 分支生成：
...
raygen 无条件执行这两个组件，而不用一次随机分支决定执行哪个：
...
选灯 PMF 只决定取哪盏灯。选中灯之后，rectangle、disk、常规 sphere 与球内/近球回退路径都在灯面上均匀取点。普通顶点的方向 PDF 为
...
对水面两份样本，上式的 $q_i$ 替换为 $q_G(i)+q_U(i)$。若它们各自选中单面 sphere 且水面交点在球外，条件采样还改为均匀可见立体角。令圆锥半角为 $\theta_{\max}$，则

$$
p_L(\boldsymbol\omega)=
\frac{q_G(i)+q_U(i)}{2\pi(1-\cos\theta_{\max})}.
$$

下面的分支先用球心距离检查可见圆锥是否有效；无效时保持旧的面积采样与 RNG 调用路径。详细几何推导见[第 5 章第 1 节](05-direct-lighting-and-mis.md#1-在灯面上取一个随机点)。
...
在一个 Lambert、GGX metal 或粗糙 GGX dielectric/water 表面上，环境 NEE 抽取方向 $\boldsymbol\omega_i$，估计
...
c_s(\mathbf n,\boldsymbol\omega_i)
}{p_E(\boldsymbol\omega_i)}w_E.
$$

$c_s$ 对 Lambert/metal 是正半球余弦，对粗糙介电反射/透射是绝对余弦。$\mathbf T$ 包含二值几何/alpha 可见性、flame 吸收，以及当前粗糙透射事件之后同一介质到无穷远的 Beer 衰减。环境 shadow 使用无限 `tmax`；任何后续透明边界都阻断当前连接，不再沿直线穿界。
...
环境与有限灯是两个独立积分域。普通表面在每个存在的域各产生一个样本，最多形成两个 connection；粗糙 water 在有限灯域固定产生功率索引与均匀索引两份样本，加上环境时最多形成三个 connection。环境与有限灯之间不再选一次类别，也不把各自 PDF 乘一个类别概率。

理论 connection 上限不等于固定的 OptiX 射线数。没有对应域、材质为 delta、方向落到不受支持的半球、PDF 无效或局部条件提前失败时不会追踪 shadow；成功进入可见性阶段的每份样本各发一次二值查询，后续透明边界直接遮挡。stats 中的 `traced_rays` 记录实际调用次数，而不是理论 connection 数。
...
粗糙水面的 $q_G/q_U$ 确定性双样本和介质内均匀覆盖是上述 $q_i^G$ 之上的固定设备端规则。粗糙水面的 BSDF 方向采样还会把 Fresnel 反射分支过采样到至少 50%，但会用实际 PDF 补偿；Lambert、metal、普通 dielectric 与 HDR 方向采样不受这条规则影响。stats 的 `render.direct_light_sampling` 保存全局基础模式，结合这套固定规则即可复现 gallery、基准和回归结果。

## 8. 验证边界

Host 测试覆盖 RGBE 原始/RLE scanline、坏 packet、尺寸上限、CDF 单调归一、最终 float 区间概率、全黑退化和有限灯代理。程序化环境生成器还必须逐字节重建 tracked HDR 资产。

GPU 定向测试检查环境唯一照明、`intensity=0` 精确黑场、旋转响应、固定 seed 确定性和 depth 1 环境 NEE；高 spp 的 uniform/importance 均值必须收敛，低 spp 下热点环境与强弱多灯的 ROI MSE 必须由 importance 明显降低。有限灯测试还用两盏不等功率且绑定可见 emitter 几何的面积灯，在 depth 2 同时触发 NEE 与 BSDF-hit 路径，验证两侧 MIS 都使用同一 $q_i$。水面 fixture 另按等散射阶数（bound depth 2 / unbound depth 3）验证确定性双选灯、单面 sphere 可见立体角和反射分支过采样后的收敛均值不变。环境 fixture 同时进入 memcheck、initcheck 和 racecheck。

重要性采样不是“画面增强”开关：它不能改变正确极限，也不能修复错误 HDR、缺失遮挡、错误材质或截断偏差。亮度分布也不是对任意 BSDF 都最优；极光滑 GGX 仍可能主要依赖 BSDF 采样。普通顶点和球内/近球回退仍采整个球面，只有球外粗糙 water 对单面 sphere 使用可见立体角。它解决的是常见高动态环境和强弱多灯下的样本浪费，不是所有方差来源。
