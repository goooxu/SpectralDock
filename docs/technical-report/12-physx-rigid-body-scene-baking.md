# 12　PhysX 刚体模拟与 Python 场景即时构建

Kinetic Foundry、“熔岩圣殿的机械先知”、Atelier 与 Assembly Hall 在普通
OptiX 渲染链之前增加了一段
**物理场景即时构建**：可执行 Python 程序定义 `Renderer` 资源和
`PhysicsWorld`，隔离的 PhysX 5.8.0 GPU worker 求出选定时刻，
`PhysicsResult.apply_to` 再把 typed render attachments 加入 SceneBuilder，
或由场景从经验证的 `BodyState` 手动实例化完整角色网格。整个过程不产生、
读取或回放场景 JSON。

PhysX 是这四个物理程序的核心构建组件，但它不链接进 Renderer native extension，
也不在 `optixLaunch` 内运行。画面仍是单帧离线路径追踪；“即时”表示每次执行
物理场景程序都 fresh 求解布局，而不是逐帧交互仿真。

![熔岩圣殿的机械先知在第 24 步的 4K 正式渲染结果](../gallery/lava-temple-oracle.avif)
...
CUDA 12.8，而同一条用户命令的父进程加载 CUDA 13.x Renderer extension；
...
Python 场景描述和 PhysX CPU dispatcher 仍然在宿主侧运行，dispatcher 只做
host task scheduling。该能力契约不检查 GPU 产品名，也不要求 RT Core。
...
`verify_gpu_contract` 在创建后和模拟阶段复查 context manager、CPU dispatcher
identity、GPU broadphase、TGS、GPU dynamics、PCM、stabilization、fatal error
与 fallback signal。PCM 跨时间步维护 contact manifold；TGS 在一个外层时间步内形成更细的求解
...
Kinetic 的步数为 $N_K=300$，熔岩圣殿专题为 $N_L=24$，Atelier 为
$N_A=480$，Assembly Hall 为 $N_H=36$，所以
...
步数；每步随后重新验证 GPU scene flags/context 并采样 GPU heap 证据，末尾要求
完整 broadphase、narrowphase、solver 与 simulation 统计非零。固定步长不自动提供
跨 GPU 确定性。“撞击峰值”和“爆发瞬间”是经过候选图选择的构图时刻，不宣称
是总动能、速度或接触冲量的数学最大值。
...
| 熔岩圣殿机械部件 | box、capsule、sphere、compound shapes | rectangle、cylinder、disk、sphere |
| 熔岩圣殿水池 | 不存在 | 解析 `water_surface` 与 Beer 吸收 |
...
熔岩圣殿专题有三段祭坛火焰、两段近黑烟代理和一段冷色神光，共六个 flame；另有
一盏 directional 和四盏符文 point lights。48 颗火星与齿轮 core 是可见
emitter geometry，不注册为额外 NEE lights。第 10 章解释 flame 的
吸收—自发光模型；它不包含散射、燃烧化学或流体输运。

右侧水池使用第 11 章的解析波面和 RGB Beer 吸收，不是 PhysX particle、
...
GPU backend 标识、有效 context、GPU dynamics/broadphase、CPU dispatcher
角色、GPU heap statistics、body 身份/顺序、单位四元数、attachment 数量、
类型与归属。
...
该专题不从某个相机角度猜测“像不像爆炸”，而是从 130 个 `BodyState` 计算
位移、速度、角速度和象限覆盖：
...
### 9.3 Atelier 与 Assembly Hall 的构图判据

Atelier 的 14 个 actor 包含 9 块彩砖、2 个球和 3 个角色 box proxy。砖与球
使用 typed attachments；三个完整角色网格由最终 `BodyState` 手动实例化。
第 480 步的 validator 要求明显下落、留在边界和水盆之外、低速度，并且大多
actor 已 sleeping。Assembly Hall 则只模拟 12 个 Spot box proxy，在第 36 步
从 `BodyState` 手动实例化完整纹理网格；至少 8 个仍在空中、至少 6 个已移动，
全部留在大厅边界内，并要求这至少 6 个已移动 actor 未 sleeping。前者选择
落定，后者选择半空倾泻。

两个封面的 spotlight、clearcoat、纹理网格灯、烟和 fBm 水面要求分别由有限
disk 面灯、现有 PBR 的 Lambert/GGX 瓣、同位 rectangle NEE 灯、吸收性 flame
和四项解析波浪近似。这些视觉替代不进入 PhysX，也不扩展 Renderer API。

## 10. 私有 IPC、持久 metadata 与可复现性

Python 用无 shell 的 `subprocess.run` 启动 worker，请求和结果路径都位于同一
`TemporaryDirectory`。private IPC v2 使用显式 little-endian 编码；worker
失败、没有写结果、协议版本错误、CUDA/PhysX 版本不符或 body 顺序改变都会
硬失败。
...
被接受的 primary result 可原子写为同 stem `.physics.json`。它固定使用
`schema_version=2` 与 `generator=spectraldock.physics/2`，除版本和 actor
状态外还记录 `cpu_dispatcher_role=host-task-scheduling-only`，以及 samples、
total、broad phase、narrow phase、solver、simulation 六类 `gpu_heap_bytes`。
private `.sdp` 永不持久化。metadata 是 GPU-only 证据，不是缓存或 scene input。
...
- 熔岩圣殿碎片在模拟前已分离，没有 fracture、裂纹传播或动态 topology；
- Atelier/Assembly Hall 的角色使用 box collision proxy；Assembly Hall 的完整
  Spot 网格由 `BodyState` 手动实例化，不是 mesh collision；
...
- 熔岩圣殿专题的 130 actors、偏心冲量、复合齿轮和 attachments：
  [`scenes/lava-temple-oracle.py`](../../scenes/lava-temple-oracle.py)
- Atelier 的落定 validator、attachments 与角色 `BodyState` 实例：
  [`scenes/atelier.py`](../../scenes/atelier.py)
- Assembly Hall 的半空倾泻 validator 与 Spot `BodyState` 实例：
  [`scenes/assembly-hall.py`](../../scenes/assembly-hall.py)
...
[上一章：运行时解析水面](11-runtime-analytic-water.md) · [返回目录](README.md) · [下一章：边界、性能与验证](13-limitations-performance-and-validation.md)
