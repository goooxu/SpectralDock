...
这组报告解释 SpectralDock **为什么能够把一个场景变成一张图**，程序化体积火焰怎样参与光传输，以及可选的 PhysX 流程怎样先把刚体模拟烘焙成可渲染场景。重点不是命令怎么运行，而是数学模型为什么成立，以及这些公式怎样对应到当前 GPU 实现。
...
如果只想先建立渲染整体认识，可读第 1、2、4、5、8 章，再返回其余章节；第 10 章是独立的物理场景生成支线，第 11 章扩展无散射参与介质。
