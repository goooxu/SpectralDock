...
“熔岩圣殿的机械先知”复用同一模型，却把六个 flame 分成三种职责：三段
白炽至橙红体积构成祭坛火焰，两段低发光、高吸收体积代理半透明黑烟，一段
低密度冷色体积代理穹顶神光。后两类是为了研究同一吸收—自发光估计器的
工程化构图用法，不应解释为烟流、大气散射或 PhysX 流体；其固定参数来自
熔岩圣殿 PhysX 专题程序把刚体结果应用到同一个 `Renderer` 后添加 flame，体积积分仍完全由本章 CUDA 路径执行。

Atelier 用同一模型构造左后壁炉；Assembly Hall 保留一簇开放炉火，并在
高粗糙度 PBR 肋条安全罩内放置三个重叠有限 emitter 球作为暖晕替代，同时以
近零自发光、高吸收 flame 衰减天窗环境光，形成烟影外观。安全罩不发生介电
透射，暖晕也不是封闭 flame 的真实透射成像；最后一种 flame
用法只是**吸收性体积代理**：它会
产生透射衰减，却没有烟粒子散射、流体速度、浮力或时间演化，文档不把它称为
原生烟雾系统。
...
原生 SceneBuilder 限制 flame 数量为 8，并要求保守光学厚度总和不超过 64。设备端每条跟踪段最多接受 4096 个候选；超过上限或发现 $\sigma>\bar\sigma$ 都会记入计数器。主机汇总后只要任一安全计数非零就拒绝输出，避免把已知有偏或未完成的结果悄悄保存为正式图。
...
- 没有体积散射、相函数、真实烟流/CFD、时间演化或 motion blur；Assembly
  Hall 的“烟影”只来自高吸收 flame 对其他光线的衰减，安全罩内暖晕则来自
  三个有限 emitter 球。
