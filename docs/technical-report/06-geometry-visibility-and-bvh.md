...
SpectralDock 先用真实外法线和入射射线得到 `front_face`，然后定向几何法线 $\mathbf n_g$，使 `wo = -ray_direction` 与它满足正点积。原始插值法线先与几何外法线对齐半球，closest-hit 再由 `effective_shading_normal` 根据该次命中的 $\mathbf n_g$ 和 $\boldsymbol\omega_o$ 定向为 $\mathbf n_s^{\mathrm{eff}}$；设备结构中 `geometric_normal` 保存前者，`normal` 保存后者。legacy mesh 仍用真实几何法线选择 object-wide 的 `front_material` 或 `back_material`；严格映射 mesh 则让一个三角形的正反面都使用同一个槽材质。
...
`build_mesh` 分配并上传 position、normal、index、可选 UV、可选 MikkTSpace face-corner tangent 和可选逐三角形材质 ID。OptiX 的三角形遍历只需要 position 与 index，因此它们的设备指针进入 `triangleArray`；normal、UV、tangent 与材质 ID 保留在独立 CUDA buffer 中，随后通过第 7 章介绍的 hitgroup SBT data 供着色程序读取。
