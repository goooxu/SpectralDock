...
`build_mesh` 分配并上传 position、normal、index 和可选 UV。OptiX 的三角形遍历只需要 position 与 index，因此它们的设备指针进入 `triangleArray`；normal 与 UV 保留在独立 CUDA buffer 中，随后通过第 7 章介绍的 hitgroup SBT data 供着色程序读取。
...
紧随片段之后，代码还设置 index stride、geometry flag、每项构建输入的 SBT record 数和无变换格式，再把完整输入交给 `compact_gas`。mesh 的变换不烘焙进 GAS，而是在 IAS 实例上设置，所以同一份几何可以被不同对象复用。
...
于是共享同一 GAS handle 的两个 mesh 实例仍可拥有各自的 IAS transform，并落到不同对象的 SBT records 取得各自的材质、alpha 和灯索引；上传后的 `OptixInstance` 数组则作为实例型构建输入，经同一 `compact_gas` 路径生成最终 IAS。
