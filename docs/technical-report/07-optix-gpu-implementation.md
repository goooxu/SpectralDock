...
### 4.2 module 与 15 个 program groups

`create_pipeline` 读取构建期生成的 IR，创建主 module；sphere 使用 OptiX 内建求交 module。当前 pipeline 有 15 个 program group：

| 类型 | 数量 | 本项目职责 |
|---|---:|---|
| ray-generation | 1 | `__raygen__pathtrace`：每个像素的 spp 与路径反弹循环 |
| miss | 2 | radiance miss 报告“未命中”，shadow miss 返回“可见” |
| hitgroup | 12 | 6 类 primitive × 2 类射线，组合 intersection、any-hit 和 closest-hit |

六类 primitive 是 sphere、triangle、disk、cylinder、parabola、mesh。rectangle 和 sketch 在主机端变为 triangle；mesh 也使用内建三角形求交，但从 SBT 读取独立的顶点属性缓冲区。disk、cylinder 和 parabola 使用项目自定义 intersection，sphere 和 triangle/mesh 使用 OptiX 内建求交。
...
| intersection | 遇到候选 primitive | disk、cylinder、parabola 运行自定义交点代码；sphere、triangle/mesh 使用内建求交 |
...
shadow payload 0 初始为 0：若一路未命中，miss 写 1；遇到有效遮挡则用 `OPTIX_RAY_FLAG_TERMINATE_ON_FIRST_HIT` 终止并保留 0。payload 1 保存目标灯索引，使 any-hit 忽略终点处那盏灯自己的几何。shadow 禁用 closest-hit，只需要遍历、intersection/内建求交、any-hit 和 miss。

两类射线使用同一 IAS，但由 ray type、SBT stride 和 miss index 选择不同 records；pipeline 因而只需声明两个 32 位 payload values。
