...
管线链接参数明确设置为 1，并只为单层实例图预留 traversable 深度：
...
这里的 `maxTraceDepth` 约束 OptiX 程序间的追踪调用深度；`optixPipelineSetStackSize` 则为同一份 pipeline 配置栈空间和 IAS→GAS 的 traversable 深度。两者都不等于路径反弹次数。
...
`launch_parameters.pointer()` 是设备端全局参数入口，`&sbt.table` 提供程序与对象数据的分发表；末尾三个参数正是二维像素网格加固定深度 1，而不是 spp 或路径深度。
...
这段赋值把列表中的概念落到同名字段：`traversable` 是 IAS 根，三个指针是每像素输出，`width`/`height`/`spp`/`max_depth` 控制工作规模，`seed` 与 `camera` 决定采样序列和初始射线。结构体随后整体上传一次，raygen 通过设备常量 `params` 读取。

`parameters.exposure` 目前是冗余字段：设备路径没有读取 `params.exposure`。真正的曝光值在 OptiX launch 和可选降噪完成后，由主机直接把 `settings.exposure` 传给 CUDA 后处理 kernel；因此不能把这个字段理解成路径积分器实际消费的输入。
...
外层每次迭代对应一个对象的 `HitgroupData`，内层按 ray type 连续追加两条 record，所以代码顺序正好实现上式的 $2\times\text{object index}+\text{ray type}$。header 由 primitive 类别与 ray type 共同选择，`record.data` 则保留该对象自己的几何和材质数据。
