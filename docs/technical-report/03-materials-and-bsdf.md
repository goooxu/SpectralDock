...
closest-hit 先把插值法线或 normal-map 法线定向到 $\mathbf n_g$ 同半球，得到原始 $\mathbf n_s$。若有限的 $\mathbf n_s\cdot\boldsymbol\omega_o>10^{-5}$，实现原样保留它；只有观察方向落到着色切平面之后或极近处时，才沿 $\mathbf n_g$ 连续弯折到刚好可用的正余弦。若 $\mathbf n_g\cdot\boldsymbol\omega_o\le10^{-7}$、输入非有限、修正向量退化，或归一化后仍不朝向 $\boldsymbol\omega_o$，则回退到 $\mathbf n_g$。这避免在普通视角无故改变已有顶点法线，同时让剪影处的散射坐标系保持有效。

<!-- source-snippet id="effective-shading-normal" path="src/device_programs.cu" anchor="effective_shading_normal" -->
```cpp
...
上面的采样推导适用于 $\mathbf n_s^{\mathrm{eff}}\cdot\boldsymbol\omega_i>0$，也就是余弦半球 sampler 能生成的方向。若 $\mathbf n_g$ 判定某个灯方向仍在真实反射侧、但它落在有效着色法线负半球，直接光求值器改用 $|\mathbf n_s^{\mathrm{eff}}\cdot\boldsymbol\omega_i|$ 形成 `f_cos`，同时把该方向的 Lambert PDF 置 0。这个 `AbsDot` 扩展只让其他策略可求值合法几何方向，并不会让 BSDF sampler 穿过真实几何表面。
...
在标准的着色正半球域 $\mathbf n\cdot\boldsymbol\omega_i>0$，GGX BRDF 为
...
这里分母中的换行是普通乘法：即 $4\,n_o n_i$，不是加法。对 $\mathbf n_g$ 仍判为反射、但 $\mathbf n\cdot\boldsymbol\omega_i<0$ 的 NEE 方向，当前实现把分母中的 $n_i$ 与融合余弦都解释为 $|n_i|$；两者解析约消，所以 `f_cos` 仍为有限值，而只能在着色正半球生成方向的 VNDF PDF 为 0。
...
perceptual roughness factor，B 通道乘 metallic factor；设备端解析如下：

<!-- source-snippet id="resolved-pbr-material" path="src/device_programs.cu" anchor="resolved_material" -->
```cpp
...
对着色正半球内的反射方向，完整 BRDF 是 Fresnel 衰减的 Lambert 项与单次散射 GGX 项之和：
...
若 $\mathbf n_g$ 仍判为反射而方向落到着色负半球，diffuse 与 GGX 项同样以 $|n_i|$ 扩展 `f_cos`，混合 PDF 则为 0。这与上一节 pure metal 的策略支撑约定一致。

采样器以观察方向的 Schlick Fresnel 估计两个波瓣的相对亮度。令镜面估计亮度为 $l_s$、Fresnel 后的漫反射估计亮度为 $l_d$：若 $l_d\le0$，镜面概率为 1（包括两个估计都为 0 的无贡献边界）；否则若 $l_s\le0$，镜面概率为 0；两者都为正时，镜面概率为

$$
p_s=\mathrm{clamp}\!\left(\frac{l_s}{l_s+l_d},0.05,0.95\right).
$$

这个下限和上限确保两个有效波瓣都保留至少 5% 样本。采样器据此在 cosine hemisphere 与 GGX VNDF 之间选择；无论选择哪一支，返回的 PDF 都是 $p_s p_{\mathrm{GGX}}+(1-p_s)p_{\mathrm{cos}}$。这样 BSDF 延续路径、有限灯/HDR NEE 与命中端 MIS 使用完全相同的测度。

<!-- source-snippet id="pbr-lobe-mixture" path="src/device_programs.cu" anchor="pbr_specular_probability" -->
```cpp
...
`metallic=1` 时漫反射消失并退化到 legacy `metal`；`metallic=0` 仍保留 $F_0=0.04$ 的介电镜面。该模型不含微表面多重散射补偿，因此不承诺严格 furnace 能量守恒。
...
normal texture 使用 OpenGL / $+Y$ 约定：线性 texel 从 $[0,1]^3$ 映射到 $[-1,1]^3$ 后，绿色通道的正方向就是 MikkTSpace 的 $+\mathbf B$，不会像 DirectX normal map 那样翻转 Y。OBJ loader 为每个 triangle face-corner 生成 tangent 与 handedness，不把镜像 UV 缝错误地平均回共享顶点。着色器对未归一化插值的 $\mathbf T$、$\mathbf B$、$\mathbf N$ 应用该 texel；`normal_scale` 只缩放切线空间 XY，组合结果在 object space 归一化后再通过实例 inverse-transpose 进入世界空间。

<!-- source-snippet id="mikk-normal-map-transform" path="src/device_programs.cu" anchor="MikkTSpace's exact unnormalized" -->
```cpp
...
介质栈与 ray-origin offset 仍完全由 $\mathbf n_g$ 决定。PBR 的这个最终 `hit.normal` 也写入首个有效交点的 OptiX Denoiser normal guide，所以降噪引导与实际 BSDF 波瓣一致；只有光滑 delta dielectric/water 的 guide 特意改用 $\mathbf n_g$。

当前 normal map 只支持 triangle mesh。绑定阶段要求完整 UV 和有效 MikkTSpace face-corner frame：条件不满足、或把 normal-mapped PBR 绑定到解析几何时，SceneBuilder 会拒绝场景，而不是在设备端静默忽略贴图。

## 4. 介电质：从 delta 界面到粗糙微表面

玻璃、水和空气这类非导体常由折射率 $\eta$ 描述。光从介质 $i$ 进入介质 $t$ 时满足 Snell 定律：

`roughness = 0` 时界面是离散反射/折射；非零时，宏观法线之上存在 GGX 微表面法线分布。无 `water_surface` 的光滑兼容路径仍保留原有的空气外部介质与 Schlick 近似，以维持既有确定性路径与随机数序列；含水光滑路径以及全部粗糙介电路径使用非偏振精确 Fresnel。含水介质栈与有界首水面分裂见[第 11 章](11-runtime-analytic-water.md)。
...
- 含水路径维护最多四层严格 LIFO 介质栈，用 RGB Beer 吸收处理水段，并只允许普通 dielectric 绑定闭合 sphere，详见[第 11 章第 4、5 节](11-runtime-analytic-water.md#4-介质栈与严格嵌套)；
