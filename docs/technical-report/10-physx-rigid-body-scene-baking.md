...
姿态，生成器再把一个固定时刻烘焙为普通 schema v2 JSON，最后仍由同一个
...
schema v2 JSON 是边界：它只描述静态几何、变换、相机、灯和渲染设置，
不携带 PhysX 对象或库句柄。
...
PhysX 返回位置 $\mathbf p$ 与四元数 $\mathbf q$；schema v2 则保存平移、
...
输出统一舍入到最多六位小数，并通过临时文件加 rename 原子替换目标。维护者
模式还会在同一容器和设备上生成第二份 scene/sidecar，再逐字节比较：

<!-- source-snippet id="physx-byte-verification" path="scripts/generate-physx-scene.sh" anchor="verify_output=" -->
```bash
if [[ "${verify}" -eq 1 ]]; then
...
字节一致证明的是**六位量化后的产物**在当前测试机、GPU 和软件栈上重复，
不是原始内部状态逐位相同。PhysX GPU 不支持 enhanced determinism，所以
sidecar 明确记录其关闭；项目不承诺跨 GPU、驱动、CUDA、PhysX、编译器或
操作系统得到相同 JSON。
...
- Python 契约能发现越界、明显落地穿透和结构漂移，不能证明每个接触都符合
  现实材料或真实世界尺度；
- host-only CI 不执行 PhysX。正式物理验收依赖专用镜像、GPU 双生成、契约
  检查和人工构图审查。

这些边界正是烘焙设计的取舍：PhysX 只负责生成一个受约束、可复现检查的
...
[上一章：边界、性能与验证](09-limitations-performance-and-validation.md) · [返回目录](README.md)
