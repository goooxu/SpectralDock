...
判别式小于零表示错过球；等于零表示相切；大于零有两个交点。选择有效范围内最小的正根。普通路径使用 OptiX 内建 sphere primitive 完成遍历和求交；含 `water_surface` 场景的 dielectric sphere 为了保留球内射线的背面退出命中，改用自定义实心 sphere，详见[第 11 章](11-runtime-analytic-water.md#水中-dielectric-sphere-为什么使用自定义实心边界)。
...
**几何法线**由真实表面形状决定，用来判断射线命中正面还是背面。**着色法线**可由网格顶点法线插值得到，让低多边形表面看起来平滑，但它不改变真实边界。

SpectralDock 先用真实外法线和入射射线得到 `front_face`，然后定向几何法线 $\mathbf n_g$，使 `wo = -ray_direction` 与它满足正点积。普通材质把与几何外法线同半球的插值法线交给 `effective_shading_normal`；normal-mapped PBR 则先重心插值 N/T/B、应用 tangent-space normal texture 并把 mapped normal 对齐到 $\mathbf n_g$ 同半球，再执行同一 effective 修正。最终得到 $\mathbf n_s^{\mathrm{eff}}$；设备结构中 `geometric_normal` 保存前者，`normal` 保存后者，完整 MikkTSpace 变换见[第 3 章第 3.5 节](03-materials-and-bsdf.md#35-tangent-space-normal-map)。legacy mesh 仍用真实几何法线选择 object-wide 的 `front_material` 或 `back_material`；严格映射 mesh 则让一个三角形的正反面都使用同一个槽材质。

$\mathbf n_s^{\mathrm{eff}}$ 只塑造连续 Lambert/PBR/GGX 波瓣、PDF 和 `AbsDot` 余弦。$\mathbf n_g$ 决定真实反射/透射半空间、光滑 delta 界面的 Fresnel/Snell 方向、介质栈转换，以及每一条新 radiance/shadow ray 的偏移侧。Denoiser normal guide 对 opaque/粗糙表面记录 $\mathbf n_s^{\mathrm{eff}}$，对光滑 dielectric/water 记录 $\mathbf n_g$，使引导法线与实际 delta 方向一致。因此着色法线不会改变真实轮廓、求交位置或介质边界。实现没有应用 shading-normal adjoint correction，极端插值法线下不承诺严格互易性或能量守恒。

## 4. UV、纹理与 alpha cutoff

UV 把三维表面点映射到二维纹理。球、圆盘、圆柱和抛物柱面有解析映射；三角网格用重心插值。设备采样图像时执行一次纵向坐标转换，以适配 AVIF 顶部原点。
...
`build_mesh` 分配并上传 position、normal、index、可选 UV、可选 MikkTSpace face-corner tangent 和可选逐三角形材质 ID。OptiX 的三角形遍历只需要 position 与 index，因此它们的设备指针进入 `triangleArray`；normal、UV、tangent 与材质 ID 保留在独立 CUDA buffer 中，随后通过第 8 章介绍的 hitgroup SBT data 供着色程序读取。
...
紧随片段之后，代码还设置无变换格式，再把完整输入交给 `compact_gas`。tangent 与 normal/UV 一样不参与 GAS 构建，只供 normal-map 着色重建 TBN。十个材质槽不会产生十份 GAS 或十条 SBT records；它们只增加一份与 triangle 数量相同的整数缓冲。mesh 的变换不烘焙进 GAS，而是在 IAS 实例上设置，所以同一份几何可以被不同对象复用。
...
三个字段承担不同职责：`traversableHandle` 决定实例进入哪一份 GAS；`instanceId` 保留对象序号作为 OptiX 可查询的语义 ID，当前设备着色路径并不依赖它选取 record；`sbtOffset` 才负责连接第 8 章的 hitgroup SBT。项目令 `kRayTypeCount = 2`，而 `optixTrace` 把 radiance 或 shadow 的 ray type 作为 record 偏移，因此最终命中记录索引为
...
[上一章：HDR 环境与重要性采样](06-hdr-environment-and-importance-sampling.md) · [返回目录](README.md) · [下一章：OptiX/GPU 实现](08-optix-gpu-implementation.md)
