...
tools/generate_benchmark_harbor.py、
tools/generate_physx_kinetic_foundry.cpp、
tools/generate_physx_lava_temple_oracle.cpp、tools/check_physx_scene.py 和
tools/check_physx_lava_temple_oracle.py
...
同 stem 的 `.physics.json`，用于记录 PhysX 版本、设备、模拟参数和临时
场景摘要；这些 sidecar 按
...
资产按 CC0 提供；生成其 130 个动态 actor 的 C++、Python 契约检查、临时
schema v6、渲染 stats 和 physics sidecar 均是 Apache-2.0 项目材料。
