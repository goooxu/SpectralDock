...
`texture()`、`material()`、`mesh()`、`object()` 和 `light()` 返回各自的 typed handle。引用关系直接传 handle，而不是传资源名称字符串：
...
handle 只能交回创建它的同一个 `Renderer`。名称仍用于调试、统计和可读性；需要时可用 `material_handle(name)` 等查询方法取得既有 handle，但这不是跨文件注册机制。
...
- `camera(look_from=..., look_at=..., up=(0,1,0), vfov=45, aperture=0, focus_distance=...)`：`vertical_fov_degrees` 是 `vfov` 的等价长名称，二者不能同时提供。
...
- `rectangle` / `sketch`：连续角点 `p1`、`p2`、`p3`；`sketch` 还需要 `alpha_texture`；
- `disk`：`center`、`normal`、`radius`；
- `cylinder`：`base`、`axis`、`height`、`radius`；
- `parabola`：`origin`、`normal`、`focus` 和 `clip={"min": ..., "max": ...}`；
- `mesh`：由 `mesh(name, path)` 返回的 `mesh` handle，以及可选 `translate`、`rotate_degrees`、`scale`；
...
OBJ 通过 `mesh()` 加载。若材质或 alpha 使用纹理，OBJ 必须具备完整 UV。变换顺序为 $T R_z R_y R_x S$，旋转单位为度。
