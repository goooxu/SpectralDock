...
- `image`：`path=...`，`color_space="srgb" | "linear"`，以及可选的
  `wrap_u`/`wrap_v="clamp_to_edge" | "repeat" | "mirrored_repeat"`；两轴
  默认均为 `clamp_to_edge`。

所有 `image` 路径必须使用小写 `.avif`，并且只接受单帧 8 bit、非预乘 alpha
且无 ICC/gain-map 的图像。`srgb` 输入使用 BT.709 primaries 与 sRGB transfer；
`linear` 数据图还必须严格使用 BT.709 primaries、linear transfer、identity
matrix、YUV 4:4:4 full range。Radiance RGBE `.hdr` 是 environment background
的专用例外，不能登记为普通 texture 或材质数据图。
...
`base_color_texture` 可按素材实际编码声明 sRGB 或 linear。首版 normal map
