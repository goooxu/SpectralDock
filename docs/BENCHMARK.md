...
- 普通颜色纹理和材质数据图必须是受支持的 8 bit AVIF profile；自发光纹理
  还可使用严格 CICP `9/16/9` 的 10/12 bit HDR AVIF，并以 203 nit=`1.0`
  解码、上传为线性 Rec.709 float texture。HDR profile 禁止用于 PBR/alpha；
  Radiance RGBE 只以严格小写 `.hdr` 作为环境贴图输入，并在采样前转换到
  线性 Rec.709/D65。
...
- UV、平滑法线、alpha、共享 mesh GAS、逐 primitive 多材质与 custom
  primitive；
- 8 bit sRGB/linear AVIF 纹理与 10/12 bit HDR AVIF 自发光纹理；后者实际
  经过 float GPU texture 上传和着色采样，验证高于 `1.0` 的线性能量不被截断；
