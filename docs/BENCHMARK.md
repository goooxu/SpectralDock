...
的 4K 封面记录。每次命令先在同一 RTX 5090 上启动 PhysX 5.8.0 checked
GPU 生成器，以 seed 909、`1/120 s` 固定步长运行 24 步，在 0.2 秒的径向
爆发瞬间导出 130 个 actor；契约要求 GPU dynamics/broadphase、TGS、PCM、
...
Compute Sanitizer 与人工构图检查的运行记录，不是跨 GPU 物理姿态 golden
或性能门槛。
...
# 即时 PhysX 场景；每条命令都会先重新运行 PhysX GPU
./scripts/build-physx-image.sh
OPTIX_ROOT="/absolute/path/to/OptiX-SDK-9.1.0" \
  ./scripts/render-physx-scene.sh --scene kinetic-foundry --preset final
OPTIX_ROOT="/absolute/path/to/OptiX-SDK-9.1.0" \
