...
程序立即报错；不存在 CPU dynamics fallback。PhysX 与 OptiX 都只要求在所选
GPU 上实际运行，不要求特定产品名或 RT Core。

“GPU-only 物理”不等于宿主完全不做工作。Python 仍定义 actor 与视觉资源，
worker 的单线程 CPU dispatcher 仍负责 PhysX 所需的宿主调度，metadata 将其
角色固定记录为 `host-task-scheduling-only`；契约所禁止的是把 broad phase
或 rigid-body dynamics 静默换成 CPU 求解。
...
flowchart LR
    ...
    H --> I["SceneBuilder → CUDA 13.x / OptiX 9.1"]
    I --> J["10 bit Rec.2020/PQ HDR AVIF 与 render stats"]
    F --> K["可选、人类可读 .physics.json"]
~~~

上图中 `.sdp` 是内部、版本化的 little-endian 二进制 IPC；当前 protocol 为
v2。它只存在于
...
使用 CUDA 13.x，避免两个 CUDA runtime、指针、context 或 SDK 句柄进入同一
地址空间。

持久 `.physics.json` 与 IPC 完全不同。当前输出固定
`schema_version=2`、`generator=spectraldock.physics/2`，记录 scene、seed、
设备、CUDA/PhysX 版本、固定步长、GPU-only flags、actor 初末姿态/速度、
sleeping 状态与 attachment 数量。`backend.gpu_heap_bytes` 还记录 samples、
total、broad phase、narrow phase、solver 和 simulation 的 GPU heap statistics。
它不含完整 Renderer 场景，不能作为下一次渲染输入，也不能跳过 fresh 模拟。
...
| 构建目录 | 关键选项 | 工具链 | 产物 |
| --- | --- | --- | --- |
| `build/Release` | `SPECTRALDOCK_ENABLE_GPU=ON` | CUDA 13.x、OptiX 9.1 | `python/spectraldock/_native` |
...
PhysX SDK、CUDA、OptiX 以及它们的构建产物不随仓库分发。路径由宿主环境
明确给出；`containers/test/Dockerfile` 可准备双 CUDA 测试环境，但仍需按宿主
架构挂载 OptiX 与 PhysX SDK。
...
`output/examples/` 写 AVIF 与 `.stats.json`，只有两个物理程序额外写同 stem
...
维护者验收可在任意满足能力契约的 NVIDIA GPU 测试机完成：它先运行独立
GPU-only probe，验证有效 CUDA context、GPU dynamics/broadphase、CPU
dispatcher 角色和非零 GPU heap evidence；随后运行 Renderer Gallery preview、
两个物理教学程序和两个 PhysX Gallery 封面的低成本物理/渲染预览，
人工检查穿透、越界、附件方向、火焰、水池和构图；随后用一次显式
`--target-processes all` memcheck 同时检查 CUDA 13.x OptiX 根进程与隔离的
...
仓库保留旧十个教学程序的正式 gallery AVIF 与 render `.stats.json`，
其中两个旧 PhysX 教学场景还保留同次运行的 `.physics.json`。新 Gallery 的
三张综合展示和十二张 OFF/ON 对比图只提交 AVIF，不提交测试机 stats 或
physics sidecar，也不建立像素 golden 或性能基准。不得保存 private `.sdp`，
也不存在序列化后的物理场景输入。
`.physics.json` 的代码/数据结构按 Apache-2.0 提供；gallery AVIF 属于
明确列出的 CC0-1.0 视觉资产。

现有 gallery sidecar 是正式图片验收时的历史原始记录，因此保留旧聚合字段；
当前 Python API 新运行产生的是 `spectraldock.physics/2`、逐 body 状态和
