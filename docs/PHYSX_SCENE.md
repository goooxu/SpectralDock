...
SpectralDock 随后读取普通 schema v6 JSON，以 CUDA/OptiX 完成路径追踪。
...
- PhysX 源码、SDK、库、二进制和专用容器镜像均不进入项目仓库。
...
每份输出最多尝试八次；PhysX 子进程异常，或穿地、越界、结构不合约的 GPU
样本都会被丢弃，八次都失败则清理目标并令命令失败。`--verify` 在同一固定
容器镜像和设备上取得两份独立通过契约的 scene JSON 和 metadata sidecar；
它不比较两次模拟的姿态字节。
sidecar 记录 PhysX GPU backend、设备、seed、`dt=1/120`、`steps=300`、重力、
scene flags、actor 计数和结果摘要。正式 sidecar 还必须记录
`sleeping_dynamic_actors=0`，证明截帧时没有动态 actor 进入 PhysX sleeping
状态。
...
`final` 模式显式替换受版本控制的同名 PNG、渲染 stats 和物理 metadata
三件套，不增加 gallery stem 或资产数量，且仍不提交临时场景 JSON：
...
`enhanced_determinism_unsupported_on_gpu`。固定 seed、时间步、截帧步数以及
actor 创建/导出顺序约束输入和输出结构，但 GPU 接触生成与并行求解顺序仍可
使同一测试机上的最终姿态不同。因此项目不承诺同机或跨 GPU、驱动、CUDA、
PhysX 版本、编译器、操作系统的逐字节确定性。

重建记录时应完成：

1. 使用 `--verify` 独立生成两次，确认两组 scene 和 metadata 都通过契约
   验证；允许最终刚体姿态不同。
...
4. 在 RTX 5090 记录环境运行 final，逐项核对 PNG、stats 与 physics
