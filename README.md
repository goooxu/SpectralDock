...
- 所有普通图像纹理和材质数据图都使用单帧 8 bit AVIF。颜色纹理按 sRGB
  解释；normal、metallic-roughness 和 alpha 等数据图使用 full-range 4:4:4、
  linear transfer 与 identity matrix 的严格配置。编码器使用 AV1 lossless。
- Radiance RGBE `.hdr` 只保留作线性环境贴图输入；它不是普通材质纹理，也不是
  渲染输出。其他栅格格式以及非 `.avif` 渲染路径都会被拒绝。

## 平台与运行契约

项目支持 Linux x86-64 与 aarch64，不针对某个 CPU 微架构编译；随 libavif
获取的 AOM 在运行时选择当前 CPU 可用的实现。GPU 必须是驱动、CUDA 与 OptiX
共同支持的 NVIDIA GPU，但不检查产品名、compute capability 白名单或 RT Core。
OptiX 初始化和 launch 必须成功。
