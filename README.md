...
GitHub Actions 只执行无 GPU 的 C++/Python host 检查。RTX 5090 维护者验收包括 Release smoke、OptiX validation、受控光传输契约和八个静态示例预览；提供 PhysX SDK 时再运行两个物理示例及其场景契约。标准托管 runner 不承担 GPU 或 PhysX 验收。

```bash
./scripts/test.sh
# 默认验收十个示例，需要 Renderer 与 PhysX 两套 SDK
./scripts/acceptance.sh
# 没有 PhysX SDK 时，显式只验收 Renderer 与八个静态示例
...
- 不自动把 MTL 数值推断为 BSDF；支持显式 `usemtl` 槽映射，但不支持骨骼、动画、通用参与介质、燃烧化学或流体动力学。
