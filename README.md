...
- Lambert、GGX metal、光滑/粗糙 dielectric 与 emitter；粗糙介电反射和透射使用 GGX 可见法线采样，并与直接光采样、MIS 和俄罗斯轮盘配合。
- 线性 Rec.709 Radiance RGBE 环境贴图，支持经纬映射、强度和绕世界 +Y 旋转；环境 NEE 与 miss 端点通过 MIS 配对。
- 显式面积灯/球灯/火焰以功率代理为全局基础分布；粗糙水面每个顶点分别取得一份功率分布样本和一份均匀索引样本，并与 BSDF 命中组成三技术 balance MIS。水面选中单面球灯时从球外顶点均匀采可见立体角，环境则按亮度与精确 texel 立体角采样。场景可在 `importance` 与 `uniform` 基础模式间切换用于 A/B 教学对照。
...
- 固定 seed 的确定性渲染、PNG 输出和同名 `*.stats.json` 运行记录；可选导出色调映射前、未降噪的线性 RGB PFM，供收敛与能量分析使用。
...
`./scripts/render-examples.sh --preset final` 会直接覆盖八个内置场景受版本控制的 gallery PNG 和对应 stats，用于重建同一组运行记录。Ember Forge 固定使用 2048 spp、depth 12 且不降噪；Moonlit Stepwell 使用粗糙水面 NEE，固定为 512 spp、depth 12 并启用 OptiX AI Denoiser。其线性收敛检查显式关闭降噪。Radiance Pavilion 只使用 HDR 环境照明。Kinetic Foundry 使用独立的 PhysX 生成/渲染入口，不在默认八场景批处理中。完整说明见[示例画廊](docs/EXAMPLES.md)。
...
- HDR 输入只支持常见 `-Y +X` 朝向的 Radiance RGBE `.hdr`（现代 RLE 或未压缩 scanline）；不读取 OpenEXR。可选 PFM 保存原始样本均值，仍不是带元数据、色彩管理或压缩的通用 HDR 交换格式。
