...
表面路径追踪是项目的核心教学链路；程序化火焰与解析水面是高级光传输实验；PhysX 只作为可选的 GPU 刚体场景烘焙支线。仓库收录七个内置研究/展示场景所需的模型和纹理，以及 Kinetic Foundry 的 PhysX 生成记录，但不分发 CUDA、OptiX、PhysX 或其他外部 SDK。
...
- 统一的 schema v4 场景加载，包含 sphere、rectangle/sketch、disk、cylinder、parabola、OBJ mesh、程序化 flame 光源与有限解析波浪水面；rectangle/sketch 在内部使用三角形 primitive。
...
- 七个内置 1920×1080 展示场景、一个固定在 300 步（2.5 秒）撞击峰值的 PhysX 5.8.0 GPU 刚体瞬时快照、低成本 smoke fixture、host-only 测试和 RTX 5090 手工 GPU 检查流程。
...
它会完成 shell 语法检查、`SPECTRALDOCK_ENABLE_GPU=OFF` 的 CMake 构建与 CTest，并运行 `tests/test_technical_report_snippets.py`。这条 CI 路径验证主机端场景/OBJ 解析、输入语义，以及技术报告的源码片段和数学标记；它不构建 `spectraldock` 渲染可执行文件、不执行像素渲染，也不是 CPU reference renderer。GPU 环境、MIS 对照、Compute Sanitizer 与像素 golden 的范围见 [RTX 5090 运行记录](docs/BENCHMARK.md)和[技术报告第 9 章](docs/technical-report/09-limitations-performance-and-validation.md)。
...
- mesh emitter 可显示发光，但不能作为显式采样灯；显式灯为 rectangle、disk、sphere 或 schema v4 flame。
