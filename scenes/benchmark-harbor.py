#!/usr/bin/env python3
"""Benchmark Harbor: 16 shared-mesh instances over 1,024 procedural wavelets."""

import math
import random
from pathlib import Path

from spectraldock import Renderer


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_SEED = 20260707
GRID_SIDE = 32
WAVE_COUNT = GRID_SIDE * GRID_SIDE
MASCOT_COUNT = 16
WAVE_RADIUS = 0.17
WAVE_SPACING = 0.48


def _rounded(value: float) -> float:
    return round(float(value), 6)


def create_renderer(seed: int = DEFAULT_SEED) -> Renderer:
    """Build the benchmark deterministically without a generated scene document."""
    renderer = Renderer()
    renderer.integrator(
        direct_light_sampling="importance",
        clamp_direct=64.0,
        clamp_indirect=16.0,
    )
    renderer.camera(
        look_from=(15.0, 10.0, 18.0),
        look_at=(0.0, 0.1, 0.0),
        up=(0.0, 1.0, 0.0),
        vfov=35.0,
        aperture=0.035,
        focus_distance=25.4,
    )
    renderer.background(
        type="sky",
        bottom=(0.08, 0.16, 0.24),
        top=(0.015, 0.035, 0.09),
        sun_direction=(-0.4, 0.78, -0.48),
        sun_color=(2.8, 2.1, 1.45),
        sun_cos_angle=0.997,
        exposure=0.0,
    )

    water_materials = (
        renderer.material(
            name="water_0", type="metal", base_color=(0.025, 0.18, 0.28), roughness=0.16
        ),
        renderer.material(
            name="water_1", type="metal", base_color=(0.035, 0.25, 0.34), roughness=0.19
        ),
        renderer.material(
            name="water_2", type="metal", base_color=(0.04, 0.31, 0.39), roughness=0.22
        ),
        renderer.material(
            name="water_3", type="metal", base_color=(0.03, 0.22, 0.32), roughness=0.18
        ),
    )
    mascot_materials = (
        renderer.material(name="mascot_0", type="lambertian", base_color=(0.84, 0.12, 0.05)),
        renderer.material(name="mascot_1", type="lambertian", base_color=(0.95, 0.44, 0.04)),
        renderer.material(name="mascot_2", type="lambertian", base_color=(0.06, 0.34, 0.58)),
        renderer.material(name="mascot_3", type="lambertian", base_color=(0.82, 0.82, 0.76)),
    )
    mascot = renderer.mesh(
        name="mascot", path=ROOT / "assets/examples/models/capsule-mascot.obj"
    )

    rng = random.Random(seed)
    for row in range(4):
        for column in range(4):
            index = row * 4 + column
            x = -5.4 + column * 3.6
            z = -5.4 + row * 3.6
            phase = 0.41 * row + 0.63 * column
            y = 0.05 + 0.08 * math.sin(phase) + rng.uniform(-0.01, 0.01)
            renderer.object(
                name=f"mascot_{index:02d}",
                type="mesh",
                mesh=mascot,
                translate=(_rounded(x), _rounded(y), _rounded(z)),
                rotate_degrees=(
                    _rounded(rng.uniform(-1.5, 1.5)),
                    _rounded(18.0 * math.sin(phase) + rng.uniform(-2.0, 2.0)),
                    _rounded(rng.uniform(-2.0, 2.0)),
                ),
                scale=(0.88, 0.88, 0.88),
                material=mascot_materials[index % 4],
            )

    half = 0.5 * (GRID_SIDE - 1) * WAVE_SPACING
    for row in range(GRID_SIDE):
        for column in range(GRID_SIDE):
            index = row * GRID_SIDE + column
            x = column * WAVE_SPACING - half
            z = row * WAVE_SPACING - half
            wave = (
                0.055 * math.sin(0.72 * x + 0.31 * z)
                + 0.035 * math.cos(0.28 * x - 0.83 * z)
                + rng.uniform(-0.006, 0.006)
            )
            renderer.object(
                name=f"wave_{index:04d}",
                type="sphere",
                center=(_rounded(x), _rounded(-0.22 + wave), _rounded(z)),
                radius=WAVE_RADIUS,
                material=water_materials[(row + 3 * column) % 4],
            )

    renderer.light(
        name="harbor_key",
        type="rectangle",
        position=(-8.0, 12.0, -6.0),
        edge_u=(12.0, 0.0, 0.0),
        edge_v=(0.0, 0.0, 8.0),
        emission=(5.5, 4.6, 3.5),
    )
    renderer.light(
        name="harbor_fill",
        type="disk",
        position=(9.0, 7.0, 8.0),
        normal=(-0.62, -0.52, -0.58),
        radius=2.8,
        emission=(1.1, 2.2, 3.8),
    )
    return renderer


def main() -> None:
    output = ROOT / "output/examples/benchmark-harbor.png"
    create_renderer(DEFAULT_SEED).render(
        output=output,
        stats_output=output.with_suffix(".stats.json"),
        width=1920,
        height=1080,
        spp=512,
        depth=12,
        seed=DEFAULT_SEED,
        denoise=True,
    )


if __name__ == "__main__":
    main()
