...
2. 运行 ./scripts/test.sh；它会在不使用 GPU、OptiX 或 PhysX 的 host-only 容器入口中执行 shell 检查、关闭 GPU/PhysX 的 CMake、CTest 和全部 pytest。该入口不构建渲染器、不渲染图像，不应称为 CPU renderer 验收。
3. GPU、OptiX、PhysX、Compute Sanitizer 与像素 golden 由维护者在已记录的 RTX 5090 环境复核。
